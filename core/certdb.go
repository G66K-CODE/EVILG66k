	...
	if err != nil {
		...
		template := x509.Certificate{
			...
			Subject: pkix.Name{
				Country:            []string{},
				Locality:           []string{},
				Organization:       []string{"evilg66k Signature Trust Co."},
				OrganizationalUnit: []string{},
				CommonName:         "evilg66k Super-Evil Root CA",
			},
			NotBefore:             notBefore,
			NotAfter:              notAfter,
...
func (d *CertDb) SignCertificateForHost(host string, phish_host string, port int) (cert *tls.Certificate, err error) {
	...
	if phish_host == "" {
		...
		template = x509.Certificate{
			...
			Subject:               pkix.Name{Organization: []string{"evilg66k Signature Trust Co."}},
			NotBefore:             time.Now(),
